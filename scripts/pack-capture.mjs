...
try {
  ...
  const stages = [
    ['sealed', 'sealed', 0], ['gripped', 'gripped', 0], ['tear-start', 'tear', .12], ['tear-half', 'tear', .5], ['tear-complete', 'tear', 1],
  ...
  if (process.env.PACK_RESPONSIVE) for (const [name, width, height] of [['16-10', 1440, 900], ['16-9', 1600, 900], ['ultrawide', 2160, 900], ['tablet', 1024, 768], ['portrait', 390, 844], ['small', 320, 568]]) {
  ...
  console.log(JSON.stringify({ errors, warnings, resources: await page.evaluate(() => window.__holo.factory.stats()), captures: captures.map(c => ({ name: c.name, triangles: c.renderer.triangles, frameMs: c.renderer.frameMs })) }, null, 2));
  await writeFile(join(out, 'report.json'), JSON.stringify({ errors, warnings, captures }, null, 2));
