...
export function crossedFacets(light: Node<'vec3'>, tangent: Node<'vec3'>, bitangent: Node<'vec3'>,
  ...
  const hz = half.dot(normal).max(.12);
  const slope = vec2(half.dot(tangent), half.dot(bitangent)).div(hz);
  ...
  const sheet = details.rg.sub(.5).mul(u.facetTilt, 2).add(authored);
  ...
  for (const vertical of [false, true]) {
    ...
    const lattice = point.mul(u.scale).mul(vec2(.72, 1.17));
    // Offset each row independently: short facets never line up into a tiled lattice.
    const row = lattice.y.floor();
    const shifted = lattice.add(vec2(stableHash(vec2(row, 9), seed + 311), 0));
    ...
    const intrinsic = float(1).div(u.sharpness.max(1)).mul(r.mul(.7).add(.65));
    const broadening = (vertical ? lightVariance.yx : lightVariance).add(intrinsic);
    ...
    const facet = normal.add((vertical ? bitangent : tangent).mul(facetSlope.x))
      .add((vertical ? tangent : bitangent).mul(facetSlope.y)).normalize();
    const axis = (vertical ? bitangent : tangent);
    const grating = axis.sub(facet.mul(axis.dot(facet))).normalize();
    const groove = facet.cross(grating).normalize();
