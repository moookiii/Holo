...
export class WrapperPicking {
  ...
  add(source: Mesh, strip: boolean, side: number) {
    ...
    const nx = 24, ny = strip ? 5 : 20, stepY = strip ? 2 : 4;
    const rest: number[] = [], coordinates: number[] = [], indices: number[] = [];
    for (let y = 0; y <= ny; y++) for (let x = 0; x <= nx; x++) {
      const index = y * stepY * 145 + x * 6;
  ...
  raycast(ray: Raycaster, pose: WrapperPose) {
    ...
    for (const skin of this.skins) {
      ...
      for (let i = 0; i < positions.count; i++) {
        ...
        const edge = Math.max(.00001, 1 - u * u), top = ease((y0 - 2.4) / (this.tearHeight - 2.4));
        const free = pose.tear >= 1 ? 1 : pose.tear > 0 ? ease((pose.tear * 2 - 1 - u) / .2) : 0;
        let x = skin.rest[i * 3], y = skin.rest[i * 3 + 1], z = skin.rest[i * 3 + 2];
        if (skin.strip) {
          const lift = free * pose.tear;
          x += lift * .18; y += lift * ((1 - u) * .23 + .40); z += lift * (Math.sin(u * 2.4) * .40 + .48);
        } else {
          x += pose.tension * .038 * top * Math.sin(y0 * 8 + u * 12);
          y -= pose.mouth * top * edge ** .65 * (skin.side > 0 ? 1.42 : .16) + pose.collapse * .2 * edge * Math.sin(y0 * 2);
          z += skin.side * (pose.mouth * top * edge * .66 + free * top * .045 + pose.grip * .11 * Math.exp(-24 * (u + .83) ** 2) * top);
