...
export function createHoloLab(context: Context) {
  ...
  async function saveProfile(asNew: boolean) {
    try {
      ...
      persist([...context.profiles.filter(item=>item.id!==p.id),p]); state.markSaved(p); refreshProfiles(); apply(true); report('Saved in this browser · export JSON for a portable copy');
  ...
  function drawStack(){
    ...
    for(const [index,r] of (['primary','secondary','stamp'] as Region[]).entries()){
      const layer=regionLayer(state.current,r),group=section(`${String(index+2).padStart(2,'0')}  ${r==='primary'?'Primary foil':r==='secondary'?'Secondary foil':'Security stamp'}`,r===region);group.className+=' hl-region';
  ...
  function draw(){drawStack();right.replaceChildren();refreshToolbar();
    if(selectedSpecial==='maps'){drawMaps();return;}if(selectedSpecial==='ink'){drawInk();return;}
    const layer=regionLayer(state.current,region);if(!layer){region='primary';draw();return;}const spec=mechanisms.find(m=>m.id===mechanism)!;
    ...
    if(mechanism==='diffraction'){
      const pattern=section('Manufacturing pattern');const choices=[...new Set(context.profiles.map(p=>p.structure.field))].filter(field=>field!=='symbol-foil'||!!layer.structure.motif);const field=select('Manufacturing pattern',choices.map(f=>[f,f.replaceAll('-',' ')]),v=>edit(p=>{regionLayer(p,region)!.structure.field=v as typeof layer.structure.field;},true));field.value=layer.structure.field;pattern.append(field,element('p','hl-note','Pattern changes prepare a manufacturing field. Authored direction maps take precedence.'));
    ...
    const groups=new Map<string,HTMLDetailsElement>();for(const param of spec.parameters){const groupName=param.group==='diffraction'?'Optical response':param.group==='structure'?'Pattern & relief':param.group==='glints'?'Angular response':'Surface';if(!groups.has(groupName)){const g=section(groupName);groups.set(groupName,g);right.append(g);}const c=numeric(param.label,valueOf(layer,param),param.min,param.max,param.step,n=>edit(p=>setValue(regionLayer(p,region)!,param,n)),start,commit);groups.get(groupName)!.append(c.row);}
    if(mechanism==='relief'&&region==='primary'){const maps=section('Authored surface response');for(const [key,label,fallback] of [['normalScale','Normal strength',context.definition().mapSettings?.normalScale??1],['embossStrength','Authored emboss depth',context.definition().mapSettings?.embossStrength??.25]] as const){const c=numeric(label,state.current.mapSettings?.[key]??fallback,0,4,.01,n=>edit(p=>{p.mapSettings??={};p.mapSettings[key]=n;}),start,commit);maps.append(c.row);}right.append(maps);}
  ...
  function drawInk(){right.append(element('span','hl-eyebrow','PROTECTIVE / PRINTED REGIONS'),element('h2','','Metallic ink & print'),element('p','hl-note','Protected print is baked into the packed coverage from the authored protection map. Original artwork and maps remain untouched.'));
    const ink=state.current.metallicInk;for(const [key,label,fallback] of [['metalness','Metallic ink response',.85],['roughness','Metallic ink roughness',.28]] as const){const c=numeric(label,ink?.[key]??fallback,0,1,.01,n=>edit(p=>{p.metallicInk??={metalness:.85,roughness:.28};p.metallicInk[key]=n;}),()=>state.begin(),()=>state.commit());right.append(c.row);}
  ...
  const beforeUnload=(event: BeforeUnloadEvent)=>{if(state.dirty||[...drafts.values()].some(d=>d.dirty)){event.preventDefault();event.returnValue='';}};window.addEventListener('beforeunload',beforeUnload);
